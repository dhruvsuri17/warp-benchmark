"""Experiment A: Edge Feature Updates at every message-passing layer.

Change vs baseline HetGNNKKT:
- Edge encoders: Linear(9→H) for ac_line, Linear(11→H) for transformer, zeros for gen/load links
- At each layer, BEFORE node aggregation, update edges: e' = MLP(h_s || h_r || e), e = e + LN(e')
- Then aggregate UPDATED edges to nodes
...
log = logging.getLogger("ExpA-Edge")
...
class HetGNNLayerEdge(nn.Module):
    """Message-passing layer with explicit edge updates before node aggregation."""

    def __init__(self, h):
        super().__init__()
        # Edge update MLPs: input = (h_sender, h_receiver, edge_embedding) → h
        self.edge_upd_ac = _mlp(2*h + h, h, h)
        self.edge_upd_xfmr = _mlp(2*h + h, h, h)
        self.edge_upd_g2b = _mlp(2*h + h, h, h)
        self.edge_upd_b2g = _mlp(2*h + h, h, h)
        self.edge_upd_l2b = _mlp(2*h + h, h, h)

        self.edge_norm_ac = nn.LayerNorm(h)
        self.edge_norm_xfmr = nn.LayerNorm(h)
        self.edge_norm_g2b = nn.LayerNorm(h)
        self.edge_norm_b2g = nn.LayerNorm(h)
        self.edge_norm_l2b = nn.LayerNorm(h)
    ...
    def forward(self, hb, hg, hl, e_ac, e_xfmr, e_g2b, e_b2g, e_l2b, data):
        ...
        if ("bus", "ac_line", "bus") in data.edge_types:
            ...
            s, d = ei[0], ei[1]
            e_upd = self.edge_upd_ac(torch.cat([hb[s], hb[d], e_ac], -1))
            e_ac = e_ac + self.edge_norm_ac(e_upd)
            mb.scatter_add_(0, d.unsqueeze(1).expand_as(e_ac), e_ac)
            mb.scatter_add_(0, s.unsqueeze(1).expand_as(e_ac), e_ac)

        if ("bus", "transformer", "bus") in data.edge_types:
            ei = data["bus", "transformer", "bus"].edge_index
            s, d = ei[0], ei[1]
            e_upd = self.edge_upd_xfmr(torch.cat([hb[s], hb[d], e_xfmr], -1))
            e_xfmr = e_xfmr + self.edge_norm_xfmr(e_upd)
            mb.scatter_add_(0, d.unsqueeze(1).expand_as(e_xfmr), e_xfmr)
            mb.scatter_add_(0, s.unsqueeze(1).expand_as(e_xfmr), e_xfmr)

        ei = data["generator", "generator_link", "bus"].edge_index
        e_upd = self.edge_upd_g2b(torch.cat([hg[ei[0]], hb[ei[1]], e_g2b], -1))
        e_g2b = e_g2b + self.edge_norm_g2b(e_upd)
        mb.scatter_add_(0, ei[1].unsqueeze(1).expand_as(e_g2b), e_g2b)

        ei_l = data["load", "load_link", "bus"].edge_index
        e_upd = self.edge_upd_l2b(torch.cat([hl[ei_l[0]], hb[ei_l[1]], e_l2b], -1))
        e_l2b = e_l2b + self.edge_norm_l2b(e_upd)
        mb.scatter_add_(0, ei_l[1].unsqueeze(1).expand_as(e_l2b), e_l2b)

        mg = torch.zeros(hg.shape[0], h, device=hg.device)
        ei_bg = data["bus", "generator_link", "generator"].edge_index
        e_upd = self.edge_upd_b2g(torch.cat([hb[ei_bg[0]], hg[ei_bg[1]], e_b2g], -1))
        e_b2g = e_b2g + self.edge_norm_b2g(e_upd)
        mg.scatter_add_(0, ei_bg[1].unsqueeze(1).expand_as(e_b2g), e_b2g)
...
class HetGNNKKT_EdgeUpdate(nn.Module):
    def __init__(self, hidden_dim=128, num_layers=8):
        ...
        self.layers = nn.ModuleList([HetGNNLayerEdge(h) for _ in range(num_layers)])
    ...
    def forward(self, data):
        ...
        for layer in self.layers:
            hb, hg, hl, e_ac, e_xfmr, e_g2b, e_b2g, e_l2b = layer(
...
def main():
    ...
    model = HetGNNKKT_EdgeUpdate(hidden_dim=args.hidden_dim, num_layers=args.num_layers).to(DEVICE)
    ...
    ckpt_path = "ckpt/exp_a_edge_best.pt"
    ...
    log.info(f"\n{'='*60}")
    log.info(f"  ExpA Edge-Update RESULTS ({len(cold_i)} instances)")
    log.info(f"{'='*60}")
