"""Experiment F: No physics loss (pure diffusion), test if physics hurts."""
import os, sys
...
log.info("=" * 60)
log.info("  EXP-F: No physics loss (lam_phy=0), pure DDPM, 30+30 epochs")
log.info("=" * 60)
...
torch.save(det_model.state_dict(), "ckpt/exp_f_det.pt")

warp_model, warp_diff = train_warp(
    case=CASE, epochs=30, hidden_dim=128, num_layers=6,
    num_groups=5, lr=1e-4, lam_phy=0.0,
)
