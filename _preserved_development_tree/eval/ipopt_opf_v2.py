"""Direct IPOPT OPF solver v2 — sparse Jacobians, proper warm-start.

Key changes from v1:
- Sparse Jacobian via jacobianstructure() + sparse data return
- Hessian approximation via L-BFGS (hessian_approximation=limited-memory)
- Proper warm-start with (x, lam_g, zl, zu) all settable
"""
import numpy as np
import cyipopt
...
class SparseOPF(cyipopt.Problem):
    """OPF as a cyipopt Problem with sparse Jacobian structure."""
    ...
    def __init__(self, om, ppopt):
        ...
        # Evaluate constraints once to get dimensions and sparsity
        ...
        # Compute sparsity structure — use union from multiple sample points
        patterns = []
        for x_sample in [x0, (xmin + xmax) / 2, xmin + 0.1 * (xmax - xmin)]:
            x_s = np.clip(x_sample, xmin + 1e-8, xmax - 1e-8)
            try:
                _, _, dh, dg = opf_consfcn(x_s, om, Ybus, self.Yf_il, self.Yt_il, ppopt, self.il)
                dg_t = dg.T.tocsr() if sparse.issparse(dg) else sparse.csr_matrix(np.asarray(dg).T)
                dh_t = dh.T.tocsr() if sparse.issparse(dh) else sparse.csr_matrix(np.asarray(dh).T)
                J = sparse.vstack([dg_t, dh_t]).tocoo()
                patterns.append(set(zip(J.row.tolist(), J.col.tolist())))
            except Exception:
                pass
        ...
        self._nnz = len(self._jac_rows)

        # Hessian sparsity structure (lower triangle)
        lmbda = {"eqnonlin": np.ones(self.n_eq), "ineqnonlin": np.ones(self.n_ineq)}
        x0_safe = np.clip(x0, xmin + 1e-6, xmax - 1e-6)
        H = opf_hessfcn(x0_safe, lmbda, om, Ybus, self.Yf_il, self.Yt_il, ppopt, self.il)
        if sparse.issparse(H):
            H = sparse.tril(H).tocoo()
        else:
            H = sparse.tril(sparse.csr_matrix(H)).tocoo()
        self._hess_rows = H.row.astype(np.intc)
        self._hess_cols = H.col.astype(np.intc)
        self._hess_nnz = len(self._hess_rows)
    ...
    def jacobian(self, x):
        _, _, dhn, dgn = opf_consfcn(x, self.om, self.Ybus,
                                      self.Yf_il, self.Yt_il, self.ppopt, self.il)
        dgn_t = dgn.T.tocsr() if sparse.issparse(dgn) else sparse.csr_matrix(np.asarray(dgn).T)
        dhn_t = dhn.T.tocsr() if sparse.issparse(dhn) else sparse.csr_matrix(np.asarray(dhn).T)
        J = sparse.vstack([dgn_t, dhn_t]).tocsc()
        vals = np.array(J[self._jac_rows, self._jac_cols]).flatten()
        return vals
    ...
    def hessian(self, x, lagrange, obj_factor):
        ...
        if sparse.issparse(H):
            H = sparse.tril(H).tocsc()
        else:
            H = sparse.tril(sparse.csr_matrix(H)).tocsc()
        return np.array(H[self._hess_rows, self._hess_cols]).flatten()
...
def solve_opf(om, ppopt, x0=None, lam_g0=None, zl0=None, zu0=None,
              warm_start=False, mu_init=1e-1, max_iter=200, print_level=0):
    """Solve OPF via IPOPT.

    Args:
        x0: primal variables [n]
        lam_g0: constraint multipliers [m] (equality + inequality)
        zl0: lower bound multipliers [n]
        zu0: upper bound multipliers [n]
        warm_start: enable IPOPT warm-start mode
        mu_init: initial barrier parameter
    """
    nlp = SparseOPF(om, ppopt)
    ...
    n_iters = len(nlp.iter_objectives) if hasattr(nlp, 'iter_objectives') else -1
