"""Exp G-nores: EPD-GNN with binding-mask loss, NO node residuals.

Same as Exp G but removes residual connections in node updates.
Node update becomes: h_new = NodeMLP(h, agg) instead of h + NodeMLP(h, agg)
Edge residuals are kept.
"""
...
log = logging.getLogger("EXP-G-NORES")
...
class EPDGNNNoRes(nn.Module):
    """EPDGNN with no node residuals in INBlock."""
    def __init__(self, hidden_dim=128, k_steps=15):
        ...
        self.h = h
        self.blocks = nn.ModuleList([INBlockNoRes(h) for _ in range(k_steps)])
...
def main():
    ...
    model = EPDGNNNoRes(hidden_dim=args.hidden_dim, k_steps=args.k_steps).to(DEVICE)
    ...
    for epoch in range(args.epochs):
        ...
        if vl < best_val:
            best_val = vl; torch.save(model.state_dict(), "ckpt/exp_gnores_best.pt")

    log.info(f"Best val: {best_val:.6f}")

    log.info(f"=== IPOPT BENCHMARK ===")
    model.load_state_dict(torch.load("ckpt/exp_gnores_best.pt", map_location=DEVICE, weights_only=True)); model.eval()
