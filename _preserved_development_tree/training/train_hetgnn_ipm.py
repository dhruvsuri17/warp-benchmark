...
from models.hetgnn_ipm import HetGNNIPM
...
def main():
    ...
    model = HetGNNIPM(hidden_dim=args.hidden_dim, num_layers=args.num_layers).to(DEVICE)
    ...
    for epoch in range(args.epochs):
        ...
        if vl < best_val:
            best_val = vl
            torch.save(model.state_dict(), "ckpt/hetgnn_ipm_best.pt")

    log.info(f"Best val loss: {best_val:.6f}")

    # IPOPT benchmark
    log.info(f"=== IPOPT BENCHMARK ({args.n_test_ipopt} instances) ===")
    model.load_state_dict(torch.load("ckpt/hetgnn_ipm_best.pt", map_location=DEVICE, weights_only=True))
    model.eval()
