"""Exp G: EPD-GNN with binding-mask loss for bound multipliers.

Same architecture as Exp E. Changes:
- Split zl/zu loss into binding vs non-binding components
- Binding constraints: MSE on multiplier value
- Non-binding: just push toward zero (lighter penalty)
"""
...
def binding_mask_loss(bp, gp, bt, gt):
    """Split loss: primals + equality duals + binding-aware bound duals."""
    # Primals
    bus_primal = F.mse_loss(bp[:, :2], bt[:, :2])
    gen_primal = F.mse_loss(gp[:, :2], gt[:, :2])

    # Equality duals (lam_P, lam_Q) — always active
    lam_loss = F.mse_loss(bp[:, 2:4], bt[:, 2:4])

    # Bound duals (zl, zu) — binding-mask
    z_pred = torch.cat([bp[:, 4:8].reshape(-1), gp[:, 2:6].reshape(-1)])
    z_true = torch.cat([bt[:, 4:8].reshape(-1), gt[:, 2:6].reshape(-1)])

    is_binding = z_true.abs() > 0.1  # in normalized space
    if is_binding.any():
        z_binding_loss = F.mse_loss(z_pred[is_binding], z_true[is_binding])
    else:
        z_binding_loss = torch.tensor(0.0, device=bp.device)

    z_nonbinding_loss = z_pred[~is_binding].pow(2).mean() if (~is_binding).any() else torch.tensor(0.0, device=bp.device)

    primal = bus_primal + gen_primal
    dual = lam_loss + z_binding_loss + 0.1 * z_nonbinding_loss
    return primal + dual, primal.item(), dual.item(), is_binding.float().mean().item()
...
def main():
    ...
    for epoch in range(args.epochs):
        ...
        if vl < best_val:
            best_val = vl; torch.save(model.state_dict(), "ckpt/exp_g_best.pt")

    log.info(f"Best val: {best_val:.6f}")

    # IPOPT benchmark (same pattern)
    log.info(f"=== IPOPT BENCHMARK ===")
    model.load_state_dict(torch.load("ckpt/exp_g_best.pt", map_location=DEVICE, weights_only=True)); model.eval()
