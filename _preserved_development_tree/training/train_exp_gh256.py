"""Exp G-h256: EPD-GNN with binding-mask loss, hidden_dim=256.

Same architecture and loss as Exp G, just wider (256 vs 128).
"""
...
log = logging.getLogger("EXP-G-H256")
...
def main():
    ...
    for epoch in range(args.epochs):
        ...
        if vl < best_val:
            best_val = vl; torch.save(model.state_dict(), "ckpt/exp_gh256_best.pt")

    log.info(f"Best val: {best_val:.6f}")

    log.info(f"=== IPOPT BENCHMARK ===")
    model.load_state_dict(torch.load("ckpt/exp_gh256_best.pt", map_location=DEVICE, weights_only=True)); model.eval()
