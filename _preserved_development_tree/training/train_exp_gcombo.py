"""Exp G-combo: EPD-GNN with no node residuals + bidirectional messages + binding-mask loss.

Kitchen-sink experiment combining the best changes:
- No node residuals (from G-nores)
- Bidirectional message passing on gen_link and load_link (from G-bidir)
- Binding-mask loss (from Exp G)
"""
...
log = logging.getLogger("EXP-G-COMBO")
...
class INBlockCombo(nn.Module):
    """IN block: no node residuals + bidirectional messages."""
    def __init__(self, h):
        super().__init__()
...
class EPDGNNCombo(nn.Module):
    """EPDGNN: no node residuals + bidirectional messages."""
    def __init__(self, hidden_dim=128, k_steps=15):
        ...
        self.h = h
        self.blocks = nn.ModuleList([INBlockCombo(h) for _ in range(k_steps)])
...
def main():
    ...
    model = EPDGNNCombo(hidden_dim=args.hidden_dim, k_steps=args.k_steps).to(DEVICE)
    ...
    for epoch in range(args.epochs):
        ...
        if vl < best_val:
            best_val = vl; torch.save(model.state_dict(), "ckpt/exp_gcombo_best.pt")

    log.info(f"Best val: {best_val:.6f}")

    log.info(f"=== IPOPT BENCHMARK ===")
    model.load_state_dict(torch.load("ckpt/exp_gcombo_best.pt", map_location=DEVICE, weights_only=True)); model.eval()
