"""Experiment A: Longer DetGNN + WARP training with baseline hyperparams."""
import os, sys
...
log.info("=" * 60)
log.info("  EXP-A: Longer training (50+30 epochs), H=128, L=6")
log.info("=" * 60)

CASE = "pglib_opf_case118_ieee"

det_model = train_det(
    case=CASE, epochs=50, hidden_dim=128, num_layers=6,
    num_groups=5, lr=3e-4,
)
torch.save(det_model.state_dict(), "ckpt/exp_a_det.pt")

warp_model, warp_diff = train_warp(
    case=CASE, epochs=30, hidden_dim=128, num_layers=6,
    num_groups=5, lr=1e-4, lam_phy=0.1,
)
